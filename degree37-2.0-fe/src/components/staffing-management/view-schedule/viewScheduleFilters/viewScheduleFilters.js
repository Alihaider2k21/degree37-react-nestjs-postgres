...
function ViewScheduleFilters({
  ...
  const getFiltersCode = async () => {
    try {
      /* CHANGE THE API CALL HERE, SET IT TO CORRECT URL PROVIDED FROM THE BACKEND */
      const result = await makeAuthorizedApiRequest(
        'GET',
  ...
  const postSelectedFilter = async () => {
    try {
      ...
      const apiPayload = {
        ...
        application_code: 'view_schedule',
  ...
  const handleArchiveFilter = async (name) => {
    /* change the URL to /view-schedule or whatever is provided from the backend. */
    const result = await makeAuthorizedApiRequest(
      'POST',
