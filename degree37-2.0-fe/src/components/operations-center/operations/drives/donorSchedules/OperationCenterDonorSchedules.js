  ...
  useEffect(() => {
    ...
    const allPdf = () => {
      newDonorSchedules?.shift_slots?.map((item, index, array) => {
        ...
        setCsvPDFData((prev) => [
          ...prev,
          `${moment(item.start_time.slice(0, 19)).format('hh:mm A')},${
    ...
    const allCsv = () => {
      newDonorSchedules?.shift_slots?.map((item, index, array) => {
        ...
        setCsvData((prev) => [
          ...prev,
          {
            slot_start_time: moment(item.start_time.slice(0, 19)).format(
              'hh:mm A'
            ),
            first_name:
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        const startTimeOptionData = updatedResponse?.shift_slots
          ...
          ?.map((item) => {
            return {
              label: moment(item.start_time.slice(0, 19)).format('hh:mm A'),
        ...
        const endTimeOptionData = updatedResponse?.shift_slots
          ...
          ?.map((item) => {
            return {
              label: moment(item.end_time.slice(0, 19)).format('hh:mm A'),
              value: item.end_time,
            };
          });
    ...
    <div className="mainContent">
      <div className="">
        <div className="filterBar p-0 mb-3">
          <div className="flex justify-content-end mb-0 position-relative">
