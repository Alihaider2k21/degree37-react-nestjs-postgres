...
function ContactVolunteerFilters({
  setIsLoading,
  fetchAllFilters,
  selectedOptions,
  setSelectedOptions,
  ...
  const getFilters = async () => {
    try {
      const result = await makeAuthorizedApiRequest(
        'GET',
        `${BASE_URL}/filters/single/crm_volunteer`
      );
  ...
  const getFiltersCode = async () => {
    try {
      const result = await makeAuthorizedApiRequest(
        'GET',
        `${BASE_URL}/filters/crm_volunteer`
      );
  ...
  const postSelectedFilter = async () => {
    try {
      ...
      const apiPayload = {
        ...
        application_code: 'crm_volunteer',
  ...
  const handleArchiveFilter = async (name) => {
    const result = await makeAuthorizedApiRequest(
      'POST',
      `${BASE_URL}/filters/delete/${deleteId}/crm_volunteer`
    );
    await result.json();
    if (result.ok || result.status === 200) {
...
export default ContactVolunteerFilters;
