...
const EditLocationNoteSubCategory = () => {
  ...
  const fetchNoteCategories = async () => {
    let url = `/${crmAdminType}/${categoryType}-category?status=true`;
  ...
  const confirmArchive = () => {
    fetchData(`/locations/note-subcategory/${id}`, 'PATCH')
      .then((res) => {
        setModalPopUp(false);
        setTimeout(() => {
...
export default EditLocationNoteSubCategory;
