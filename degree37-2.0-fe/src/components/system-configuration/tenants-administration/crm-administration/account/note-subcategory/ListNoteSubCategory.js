...
const ListNoteSubCategory = () => {
    ...
    if (itemToArchive) {
      try {
        const result = await fetchData(
          `/accounts/note-subcategory/${itemToArchive}`,
          'PATCH'
        );
        const { status_code, response } = result;
        if (status_code === 204) {
          setModalPopUp(false);
  ...
  const handleAddClick = () => {
    navigate(
      '/system-configuration/tenant-admin/crm-admin/accounts/note-subcategories/create'
    );
  };
  const getData = async () => {
    setIsLoading(true);
    const result = await fetchData(
      `/accounts/note-subcategory?limit=${limit}&page=${currentPage}&is_active=${
        isActive?.value ?? ''
  ...
  useEffect(() => {
    const handleSearchChange = async (e) => {
      try {
        setIsLoading(true);
        const result = await fetchData(
          `/accounts/note-subcategory?is_active=${
            isActive?.value ?? ''
          }&name=${e}`
        );
  ...
  useEffect(() => {
    const getWithStatus = async () => {
      setIsLoading(true);
      const result = await fetchData(
        `/accounts/note-subcategory?limit=${limit}&page=${1}&is_active=${
          isActive?.value ?? ''
        }&name=${searchText}`
      );
      const { data, status_code } = result;
  ...
  const optionsConfig = [
    CheckPermission([
      ...
      label: 'View',
      path: (rowData) =>
        `/system-configuration/tenant-admin/crm-admin/accounts/note-subcategories/${rowData.id}`,
      action: (rowData) => {},
    },
    CheckPermission([
      ...
      label: 'Edit',
      path: (rowData) =>
        `/system-configuration/tenant-admin/crm-admin/accounts/note-subcategories/${rowData.id}/edit`,
      action: (rowData) => {},
    },
    CheckPermission([
