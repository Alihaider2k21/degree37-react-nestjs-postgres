...
const NoteCategory = () => {
  ...
  useEffect(() => {
    const getData = async (id) => {
      try {
        setIsLoading(true);
        if (id) {
          const result = await fetchData(`/accounts/note-category/${id}`);
  ...
  const BreadcrumbsData = [
    ...AccountBreadCrumbsData,
    {
      ...
      class: 'active-label',
      link: `/system-configuration/tenant-admin/crm-admin/accounts/note-categories/${id}`,
    },
  ];
  ...
  return (
    <ViewForm
      ...
      editLink={
        ...
        `/system-configuration/tenant-admin/crm-admin/accounts/note-categories/${id}/edit`
      }
