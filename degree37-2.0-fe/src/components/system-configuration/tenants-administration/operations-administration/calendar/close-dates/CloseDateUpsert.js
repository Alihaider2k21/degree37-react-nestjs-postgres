...
import styles from './close-date.module.scss';
...
const CloseDateUpsert = ({ closeDateId }) => {
  ...
  const BreadcrumbsData = [
    ...
    {
      label: closeDateId ? 'Edit Close Date' : 'Create Close Date',
      class: 'active-label',
      link: closeDateId
        ? `/system-configuration/tenant-admin/operations-admin/calendar/close-dates/${closeDateId}/edit`
  ...
  const getCloseDateData = async () => {
    ...
    if (result.ok || result.status === 200) {
      setCloseDateData(data.closeDate);
    ...
    if (!Object.values(err).some((value) => value !== '')) {
      try {
        ...
        const res = await makeAuthorizedApiRequest(
          closeDateId ? 'PUT' : 'POST',
          closeDateId
            ? `${BASE_URL}/close-dates/${closeDateId}`
            : `${BASE_URL}/close-dates`,
          JSON.stringify(body)
        );
        ...
        if (status === 'success') {
          ...
          toast.error('Failed to create close date.', { autoClose: 3000 });
          // Handle bad request
        } else {
          toast.error('Failed to create close date.', { autoClose: 3000 });
        }
  ...
  return (
    <div className="mainContent">
      <TopBar
        ...
        BreadCrumbsTitle={closeDateId ? 'Edit Close Date' : 'Create Close Date'}
        SearchPlaceholder={null}
        SearchValue={null}
        SearchOnChange={null}
      />
      <div className="mainContentInner mt-5">
        <form className={styles.closeDate}>
          <div className="formGroup mt-5">
            <h5>{closeDateId ? 'Edit Close Date' : 'Create Close Date'}</h5>
...
export default CloseDateUpsert;
